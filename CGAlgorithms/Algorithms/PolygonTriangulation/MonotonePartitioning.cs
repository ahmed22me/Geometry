...
namespace CGAlgorithms.Algorithms.PolygonTriangulation
{
    ...
    class MonotonePartitioning : Algorithm
    {
        ...
        int index_of_mini_x;
        public static bool IS_points_Sorted_CCW(CGUtilities.Point Min_X, int ind_OF_Min_X, List<Point> points)
        {
            // elly_b3dy rotational =( i + 1 ) % count ;
            ...
            CGUtilities.Line l1 = new Line(prev, next);
        ...
        public override void Run(List<Point> points, List<Line> lines, List<Polygon> polygons, ref List<Point> outPoints, ref List<Line> outLines, ref List<Polygon> outPolygons)
        {
            ...
            /// First at all we must check That poly is sorted CCW
            ...
            if (Points_sorted_CCW == false)
            {
                /*  for (int i = 0; i < polygons[0].lines.Count; i++)
                    {
                        Point item = polygons[0].lines[i].Start;

                        polygons[0].lines[i].Start = polygons[0].lines[i].End;
                        polygons[0].lines[i].End = item;
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                else if (V.Type == "nehayet_el_point")
                {
        ...
        } //nehayet_el_point of Sort_Polygon
        ...
        private string Classify_point(Point p, int index, List<Point> _polygon)
        {
            ...
            return type;
        } //nehayet_el_point of classify function 
